/*_############################################################################
  _## 
  _##  SNMP4J-Agent 3 - MutableVACM.java  
  _## 
...
public interface MutableVACM extends VACM {
  ...
  /**
   * Check if there is a security mapping for the specified security model and
   * security name.
   * @param securityModel
   *    the security model the mapping is based on, see
   *   {@link SecurityModel} for possible values.
   ...
   *    a security name.
   * @return
   *   {@code true} if there is a mapping and {@code false} otherwise.
   */
  boolean hasSecurityToGroupMapping(int securityModel, OctetString securityName);
   ...
   * @param groupName
   *    the group name for which access entries should be counted.
   * @return
   *    the number of group to view(s) access entries.
   * @since 3.0
   */
  int accessEntryCount(OctetString groupName);
   ...
   * @since 3.0
   */
  int viewTreeFamilyEntryCount(OctetString viewName);
}
