/*_############################################################################
  _## 
  _##  SNMP4J-Agent 3 - TestAgent.java  
  _## 
...
public class TestAgent extends BaseAgent {
    ...
    private SortedMap<OID, List<Object>> sparseTableData = new TreeMap<OID, List<Object>>();
...
//    snmpCommunityMIB.setSourceAddressFiltering(true);
    ...
    /*
    snmp4jDemoMib = new Snmp4jDemoMib(new DefaultMOFactory() {
      @Override
      ...
      createTableModel(OID tableOID, MOTableIndex indexDef, MOColumn[] columns) {
        if (tableOID.equals(Snmp4jDemoMib.oidSnmp4jDemoSparseEntry)) {
          ArrayList<Integer> colSyntaxes = new ArrayList<Integer>(51);
          colSyntaxes.add(SMIConstants.SYNTAX_UNSIGNED_INTEGER32);
          for (int i=1; i<51; i++) {
            colSyntaxes.add(SMIConstants.SYNTAX_OCTET_STRING);
          }
          M tableModel = (M) new SimpleMOTableModel<Object>(colSyntaxes,
              new SimpleValueConverter<Object>() {
                @Override
                public Object convertToValue(Variable variable, Object sampleValue) {
                  return convertToValue(variable);
                }

                @Override
                public Object convertToValue(Variable variable) {
                  if (variable.getSyntax() == SMIConstants.SYNTAX_OCTET_STRING) {
                ...
                @Override
                public Variable convertFromValue(Object value, int targetSmiSyntax) {
                  Variable variable = AbstractVariable.createFromSyntax(targetSmiSyntax);
                  if (variable instanceof OctetString) {
                    ((OctetString)variable).setValue(value.toString());
                  }
                  else {
                    ((Integer32)variable).setValue(value.toString());
                  }
                  return variable;
                }
              }, sparseTableData, new SimpleMOTableModel.SimpleRowListFactory<Object>() {
            @Override
            public List<Object> createRow(Variable[] values) {
              return null;
            }
          });
          return tableModel;
