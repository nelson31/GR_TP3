/*_############################################################################
  _## 
  _##  SNMP4J-Agent 3 - DefaultMOOutput.java  
  _## 
  _##  Copyright (C) 2005-2018  Frank Fock (SNMP4J.org)
  _##  
  _##  Licensed under the Apache License, Version 2.0 (the "License");
  _##  you may not use this file except in compliance with the License.
  _##  You may obtain a copy of the License at
  _##  
  _##      http://www.apache.org/licenses/LICENSE-2.0
  _##  
  _##  Unless required by applicable law or agreed to in writing, software
  _##  distributed under the License is distributed on an "AS IS" BASIS,
  _##  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
  _##  See the License for the specific language governing permissions and
  _##  limitations under the License.
  _##  
  _##########################################################################*/


package org.snmp4j.agent.io;

import java.io.*;

import org.snmp4j.smi.*;

public class DefaultMOOutput implements MOOutput {

  private ObjectOutputStream os;

  public DefaultMOOutput(ObjectOutputStream os) {
    this.os = os;
  }

  public void writeContextBegin(Context context) throws IOException {
    os.writeObject(context);
  }

  public void writeContextEnd(Context context) throws IOException {
    os.writeObject(context);
  }

  public void writeIndexedVariables(IndexedVariables indexedVariables) throws
      IOException {
    os.writeObject(indexedVariables);
  }

  public void writeManagedObjectBegin(MOInfo mo) throws IOException {
    os.writeObject(mo);
  }

  public void writeManagedObjectEnd(MOInfo mo) throws IOException {
    os.writeObject(mo);
  }

  public void writeSequence(Sequence sequence) throws IOException {
    os.writeObject(sequence);
  }

  public void writeVariable(Variable variable) throws IOException {
    os.writeObject(variable);
  }

  public void close() throws IOException {
    os.flush();
    os.close();
  }
}
