...
public class OID extends AbstractVariable
     ...
     *
     * @param prefixOID an array of {@code int} values. The array
     *                  is copied. Later changes to {@code prefixOID} will therefore not
     *                  affect the OID's value.
     * @param suffixOID an array of {@code int} values which will be appended to the
     ...
     *                  is copied. Later changes to {@code prefixOID} will therefore not
     *                  affect the OID's value.
     * @param suffixID  an {@code int} value that will be appended to the
     *                  {@code prefixOID} OID. The array is copied. Later changes to
     ...
     *
     * @param mask a mask where the n-th bit corresponds to the n-th sub-identifier.
     * @return the masked OID.
     * @since 1.5
     */
     ...
     * by {@link #OID(String)}.
     *
     * @return a formatted string representation of this OID (e.g. "ifDescr.1") that
     ...
     *
     * @return a string representation of this OID as defined by the
     ...
     *
     * @return a dotted string representation of this OID value.
     * @since 2.2
     */
     ...
     *
     * @return the sub-identifies of this {@code OID} as a byte array. Each
     ...
     *
     * @param index a zero-based index into the {@code OID}.
     * @return the sub-indentifier value at <code>index</code>. NOTE: The returned
     ...
     *
     * @param index int
     * @return the sub-identifier value at {@code index} as an unsigned long
     * value.
     */
     ...
     *
     * @param index a zero-based index into the {@code OID}.
     * @param value a 32bit unsigned integer value.
     * @throws ArrayIndexOutOfBoundsException if the index is out of range (index &lt; 0 || index &gt;= size()).
     */
     ...
     *
     * @param oid an {@code OID} instance.
     * @return a pointer to this OID instance (useful for chaining).
     */
     ...
     *
     * @return an integer value between 0 and 128.
     */
     ...
     * @param other an {@code OID} to compare with this {@code OID}.
     * @return <UL>
     ...
     *
     * @return the value of the last sub-identifier of this OID as an integer value.
     ...
     * {@link NoSuchElementException} is thrown
     *
     * @return the value of the last sub-identifier of this OID as an unsigned long.
     * @since 1.2
     */
     ...
     * and returns it.
     *
     * @return the last sub-identifier or -1 if there is no sub-identifier left in
     * this {@code OID}.
     */
     ...
     *
     * @param n the number of subidentifiers to remove. If {@code n} is zero or
     ...
     *
     * @return a copy of this OID with {@code n-1} sub-identifiers where
     ...
     *
     * @return an OID clone of this OID with a zero sub-identifier appended.
     * @since 1.7
     */
    ...
    public long[] toUnsignedLongArray() {
        ...
        for (int i = 0; i < value.length; i++) {
            longs[i] = Integer.toUnsignedLong(value[i]);
        }
        return longs;
    }
    ...
    /**
     * Returns a new OID that is a sub-sequence of this OID starting at {@code beginIndex} until the end of this OID.
     * @param beginIndex the begin index, inclusive.
     * @return the specified sub OID.
     * @since 3.4.1
     */
    public OID subOID(int beginIndex) {
        ...
        int[] sub = new int[value.length - beginIndex];
        System.arraycopy(value, beginIndex, sub, 0, value.length-beginIndex);
        return new OID(sub);
    }
    ...
    public OID subOID(int beginIndex, int endIndex) {
        if (beginIndex < 0 || endIndex < 0 || endIndex > value.length || beginIndex > endIndex) {
            throw new ArrayIndexOutOfBoundsException();
        }
        int[] sub = new int[endIndex - beginIndex];
        System.arraycopy(value, beginIndex, sub, 0, endIndex-beginIndex);
        return new OID(sub);
    }
}
