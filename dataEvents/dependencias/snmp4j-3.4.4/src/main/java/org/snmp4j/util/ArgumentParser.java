    ...
    public Map<String, List<Object>> parse(String[] args) throws ParseException {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if ((format.getParameters() != null) &&
                ...
                List<Object> values = parseValues(args, i + (1 - diff), format);
                i += Math.max(values.size() - diff, 0);
                if (format.isVariableLength() && options.containsKey(format.getOption())) {
                    options.get(format.getOption()).addAll(values);
                } else {
...
//    @SuppressWarnings("unchecked")
     ...
     *
     * @return the first option value in {@code optionValues} if it exists,
     * {@code null} otherwise.
     * @since 1.9.2
     */
     ...
     *
     * @return the {@code n}-th (zero based) option value in
     ...
     *
     * @return the command set matching the command in the argument list.
     * @throws ParseException
