...
/**
 * The {@code CommandResponderEvent} is fired by the
 * {@code MessageDispatcher} to listeners that potentially can process
...
public class CommandResponderEvent<A extends Address> extends EventObject {
     ...
     *         the {@code TransportMapping} which received the PDU.
     * @param sourceAddress
     ...
     * @param pduHandle
     *         the PDU handle that uniquely identifies the {@code pdu}.
     * @param pdu
     ...
     *         needed for responding a request, will be {@code null} for notifications.
     */
    public CommandResponderEvent(MessageDispatcher messageDispatcher,
                                 TransportMapping<? super A> transportMapping,
                                 A sourceAddress,
    ...
    /**
     * Creates shallow copy of the supplied {@code CommandResponderEvent} but the source of the event is set to the
     * supplied source.
     *
     * @param source
     *         the (new) source of event copy to create.
     * @param other
     *         the {@code CommandResponderEvent} to copy.
     *
     * @since 1.1
     */
    ...
    /**
     * Gets the message dispatcher instance that received the command (request PDU) or unconfirmed PDU like a report,
     * trap, or notification..
     *
     * @return the {@code MessageDispatcher} instance that received the command.
     */
    ...
    public void setStateReference(org.snmp4j.mp.StateReference<A> stateReference) {
        this.stateReference = stateReference;
    }

    public org.snmp4j.mp.StateReference<A> getStateReference() {
     ...
     *
     * @param processed
     *         If set to {@code true}, the dispatcher stops dispatching this event to other event listeners,
     *         because it has been successfully processed.
     */
     ...
     *
     * @return {@code true} if this event has been processed, <code>false</code>
     * otherwise.
     */
     ...
     *
     * @return the {@code Address} of the PDU sender.
     */
     ...
     *
     * @return a {@code TransportMapping} instance.
     */
     ...
     *
     * @param peerAddress
     *         the {@code Address} of the PDU sender.
     */
     ...
     *
     * @return a {@link TransportStateReference} instance if the transport and/or the security model supports it or
     * {@code null} otherwise.
     * @since 2.0
     */
    public TransportStateReference getTmStateReference() {
        if (tmStateReference == null) {
            StateReference<A> stateReference = getStateReference();
     ...
     *         the transport model (mapping) state information associated with this command responder event.
     */
