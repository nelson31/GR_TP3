...
public class Snmp implements Session, CommandResponder {
    ...
    /**
     * The {@code pendingRequests} table contains pending requests accessed trough the key {@code PduHandle}
     */
     ...
     * no security protols (by default). You will have to add those by calling the appropriate methods on {@link
    ...
    public <A extends Address> void processPdu(CommandResponderEvent<A> event) {
        ...
        if (pdu.getType() == PDU.REPORT) {
            ...
            if (request == null) {
                ...
                if (l != null) {
                    l.onResponse(new ResponseEvent<A>(this, event.getPeerAddress(), request.pdu, pdu,
                            request.userObject));
                }
            }
        } else {
     ...
     *         {@code true} to disable context engine ID discovery, {@code false} to enable context engine ID
     *         discovery.
     *
     * @since 2.0
     */
        ...
        public <A extends Address> void processReport(PduHandle handle, CommandResponderEvent<A> e) {
            ...
            if (resend) {
                ...
                if (intime && (reqListener != null)) {
                    // return report
                    reqListener.onResponse(new ResponseEvent<A>(Snmp.this, e.getPeerAddress(), reqPDU, pdu,
                            reqUserObject));
                } else {
            ...
            try {
                ...
                if (this.pendingRetry) {
                    try {
                        ...
                        if (l != null) {
                            listener.onResponse(new ResponseEvent<A>(Snmp.this, null,
