...
public class SnmpConfigurator {
    ...
    private void configureEngine(Session snmp, USM usm, Map<String,List<Object>> settings) {
        int engineBoots = 0;
        Integer bc =
    ...
    public Target<? super Address> getTarget(Map<String,List<Object>> settings) {
        String version =
        ...
        Target<? super Address> t;
        if ("1".equals(version)) {
            t = new CommunityTarget<Address>();
            ...
            CertifiedTarget<? super Address> ct = new CertifiedTarget<Address>(new OctetString(sn));
