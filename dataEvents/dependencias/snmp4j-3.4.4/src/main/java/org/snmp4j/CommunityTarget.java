...
/**
 * A {@code CommunityTarget} represents SNMP target properties for community based message processing models
 * (SNMPv1 and SNMPv2c).
 *
 * @author Frank Fock
 * @version 3.1
...
public class CommunityTarget<A extends Address> extends AbstractTarget<A> {
    ...
    /**
     * Gets the community octet string (which is the same as the security name). Thus, you can (and should) use {@link
     * #getSecurityName()} directly.
     *
     * @return an {@code OctetString} instance, should be never {@code null} when using this target to send
    ...
    /**
     * Sets the community octet sting. This is a convenience method to set the security name for community based SNMP
     * (v1 and v2c). It basically checks that the community is not {@code null} and then calls {@link
     * #setSecurityName(org.snmp4j.smi.OctetString)} with the supplied parameter.
     *
     * @param community
     *         an {@code OctetString} instance which must not be
     *         {@code null}.
     */
    ...
    public Target<A> duplicate() {
        CommunityTarget<A> target = new CommunityTarget<A>(getAddress(), getCommunity());
