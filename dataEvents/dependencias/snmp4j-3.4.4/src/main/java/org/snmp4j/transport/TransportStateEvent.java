...
/**
 * The {@code TransportStateEvent} describes a state change for
...
public class TransportStateEvent extends EventObject {
   ...
   * @return
   *    a (possibly empty) list of messages that were discarded or {@code null} if the event has not terminated
   ...
   * @return
   *    {@code true} if the event has been canceled.
   * @since 1.8
   */
   ...
   * {@link #STATE_CONNECTED} events can be canceled.
   * @param cancelled
   *    {@code true} if the event should be canceled, i.e. a connection
