...
public class USM extends SNMPv3SecurityModel {
    ...
    private final transient List<UsmUserListener> usmUserListeners = new CopyOnWriteArrayList<>();
     ...
     *
     * @return {@code true} if the user is either known for the specified engine ID
     ...
     *         a security name.
     *
     * @return an localized {@link org.snmp4j.security.UsmUserEntry} if the provided
     ...
     * </p>
     * The security name of the {@code user} must be not longer than {@link #MAXLEN_USMUSERNAME}
     * bytes.
     *
     * @param userName
     *         a user name.
    ...
    public void setUsers(UsmUser[] users) {
        if ((users == null) || (users.length == 0)) {
            ...
            ArrayList<UsmUserEntry> v = new ArrayList<>(users.length);
     ...
     *
     * @return the {@code UsmUserTable} instance containing the users known by
     * this USM.
     */
    ...
    /**
     * Returns the {@code UsmTimeTable} instance used by this USM for holding
     ...
     *         deleted.
     *
     * @return the removed {@code UsmUser} instances as a List. If the user could
     ...
     *         a user name.
     *
     * @return the removed {@code UsmUser} instance associate with the given
     * {@code userName} or {@code null} if such a user could not
     * be found.
     * @deprecated If the engineID {@code null} is provided this method does only
     * delete the generic user. All already localized users will not be deleted.
     * To delete those users too, use {@link #removeAllUsers(OctetString, OctetString)} instead.
     */
    @Deprecated
    ...
    protected void fireUsmUserChange(UsmUserEvent e) {
        List<UsmUserListener> listeners = usmUserListeners;
     ...
     *
     * @return a {@code SecurityProtocols} instance which is by default the
