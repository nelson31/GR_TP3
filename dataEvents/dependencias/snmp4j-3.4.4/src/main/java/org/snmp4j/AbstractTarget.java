...
/**
 * A {@code AbstractTarget} class is an abstract representation of a remote SNMP entity. It represents a target with an
 * Address object, as well protocol parameters such as retransmission and timeout policy. Implementers of the {@link
 * Target} interface can subclass {@code AbstractTarget} to take advantage of the implementation of common {@code
 * Target} properties.
 *
 * @author Frank Fock
 * @version 2.0
...
public abstract class AbstractTarget<A extends Address> implements Target<A> {
     ...
     *
     * @param retries
     *         the number of retries. <em>Note: If the number of retries is set to 0, then the request will be sent out
     *         exactly once.</em>
     */
     ...
     *
     * @return the maximum PDU size of request PDUs for this target. Which is always greater than 484.
     */
    ...
    /**
     * Sets the prioritised list of transport mappings to be used for this target. The first mapping in the list that
     * matches the target address will be chosen for sending new requests. If the value is set to {@code null}
     * (default), the appropriate {@link TransportMapping} will be chosen by the supplied {@code address} of the target.
     * If an entity supports more than one {@link TransportMapping} for an {@link Address} class, the the results are
     * not defined. This situation can be controlled by setting this preferredTransports list.
     *
     ...
     * @since 2.0
     */
    public void setPreferredTransports(List<TransportMapping<? super A>>
                                               preferredTransports) {
     ...
     *
     * @param securityModel
     *         an {@code int} value as defined in the {@link org.snmp4j.security.SecurityModel} interface or any third
    ...
    public boolean equals(Object o) {
        ...
        if (getRetries() != that.getRetries()) return false;
        if (getTimeout() != that.getTimeout()) return false;
        if (getMaxSizeRequestPDU() != that.getMaxSizeRequestPDU()) return false;
        if (getSecurityLevel() != that.getSecurityLevel()) return false;
        if (getSecurityModel() != that.getSecurityModel()) return false;
        if (!getAddress().equals(that.getAddress())) return false;
        if (getPreferredTransports() != null ? !getPreferredTransports().equals(that.getPreferredTransports()) : that.getPreferredTransports() != null)
            return false;
        if (getSecurityName() != null ? !getSecurityName().equals(that.getSecurityName()) : that.getSecurityName() != null)
            return false;
