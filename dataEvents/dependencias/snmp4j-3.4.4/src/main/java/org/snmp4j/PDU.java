...
public class PDU implements BERSerializable, Serializable {
    ...
    protected ArrayList<VariableBinding> variableBindings = new ArrayList<>();
     ...
     * other PDU.
     *
     * @param other the {@code PDU} to copy from.
     */
    public PDU(PDU other) {
        variableBindings = new ArrayList<>(other.size());
     ...
     * @param vbs     the variable bindings.
     * @since 2.2.4
     */
    public PDU(int pduType, List<? extends VariableBinding> vbs) {
        this.type = pduType;
        variableBindings = new ArrayList<>(vbs.size());
     ...
     * {@code null}.
     *
     * @param vb a {@code VariableBinding} instance.
     */
     ...
     *
     * @param vbs an array of {@code VariableBinding} instances. The instances in the
     ...
     * {@link #add(VariableBinding vb)}).
     *
     * @param vbs a list of {@code VariableBinding} instances. The instances in the
     ...
     *
     * @param index a zero based positive integer ({@code 0 &lt;= index &lt; {@link #size()}})
     * @return a VariableBinding instance. If {@code index} is out of bounds
     ...
     *
     * @param prefix the search {@link OID}.
     * @return the {@link Variable} of the first {@link VariableBinding}
     ...
     *
     * @param prefix the search {@link OID}.
     * @return a List of all {@link VariableBinding}s
     * whose prefix matches {@code oid}. If no such element
     ...
     *              an exception is thrown.
     * @param vb    a VariableBinding instance ({@code null} is not allowed).
     ...
     *
     * @return the internal {@link List} containing the PDU's variable bindings.
     */
     ...
     *
     * @param vbs a list of {@link VariableBinding} instances which must
     *            not be null.
     * @since 2.1
     */
    public void setVariableBindings(List<? extends VariableBinding> vbs) {
        ...
        this.variableBindings = new ArrayList<>(vbs);
    }
     ...
     *
     * @return {@code true} if {@link #getType()} returns {@link PDU#RESPONSE} or [@link PDU#REPORT} and
    ...
    public void decodeBER(BERInputStream inputStream) throws IOException {
        ...
        variableBindings = decodeVariableBindings(inputStream);
     ...
     *
     * @param variableBindings a list of variable bindings.
     * @return the length in bytes of the BER encoded VB list.
     */
    ...
    public void encodeBER(OutputStream outputStream) throws IOException {
        ...
        encodeVariableBindings(outputStream, variableBindings);
    }

    public static void encodeVariableBindings(OutputStream outputStream,
                                              List<VariableBinding> variableBindings) throws IOException {
        int vbLength = 0;
        for (VariableBinding vb : variableBindings) {
     ...
     *
     * @return an {@code Integer32} instance.
     */
     ...
     * @return a string representation of {@code type}, for example "GET".
     */
     ...
     * @return the corresponding PDU type constant, or {@code Integer.MIN_VALUE}
     ...
     *
     * @return an array of {@code VariableBinding} instances of this PDU in the
