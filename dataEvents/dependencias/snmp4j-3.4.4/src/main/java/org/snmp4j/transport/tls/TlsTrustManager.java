...
public class TlsTrustManager implements X509TrustManager {
    ...
    public void checkClientTrusted(X509Certificate[] x509Certificates, String s) throws CertificateException {
        ...
        try {
            ...
            LOGGER.warn("Client certificate validation failed for '" + x509Certificates[0] + "':"+cex.getMessage());
            throw cex;
        }
    }
    ...
    public void checkServerTrusted(X509Certificate[] x509Certificates, String s) throws CertificateException {
        ...
        if (entry != null && fingerprint != null && fingerprint.length() == 0 &&
                TransportStateReference.hasCertifiedIdentity(tmStateReference) &&
                ((CertifiedIdentity)tmStateReference.getTarget()).getIdentity() != null) {
            String dNSName = ((String) entry).toLowerCase();
            String hostName = ((CertifiedIdentity)tmStateReference.getTarget()).getIdentity().toString();
        ...
        if ((accepted != null) && (securityCallback != null)) {
            ...
            for (X509Certificate cert : accepted) {
                try {
                    ...
                    // ignore
                    LOGGER.debug("Security callback "+securityCallback+" rejected "+cert+": "+certex.getMessage());
                }
            }
    ...
    protected TlsTmSecurityCallback<X509Certificate> getSecurityCallback() {
        if (TransportStateReference.hasCertifiedIdentity(tmStateReference)) {
            if (tmStateReference.getTarget() instanceof TlsX509CertifiedTarget) {
                TlsX509CertifiedTarget<?> tlsCertifiedTarget =
                        ((TlsX509CertifiedTarget<?>) tmStateReference.getTarget());
