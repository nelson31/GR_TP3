...
public class MPv3
    ...
    public <A extends Address> int prepareOutgoingMessage(A transportAddress, int maxMessageSize,
        ...
        if (status == SnmpConstants.SNMPv3_USM_OK) {
            if (expectResponse) {
                cache.addEntry(new StateReference<A>(msgID,
                        flags,
                        maxMessageSize,
                        sendPduHandle,
                        transportAddress,
    ...
    private static int initHeaderDataFromSecurityLevel(int securityLevel) {
        ...
        switch (securityLevel) {
            ...
            case SecurityLevel.AUTH_PRIV:
                ...
                break;
            default:
                logger.warn("Unknown security level "+securityLevel+" ignored and set to authPriv(3) instead");
                flags = 3;
        }
        return flags;
    }
    ...
    @Override
    public <A extends Address> int prepareDataElements(MessageDispatcher messageDispatcher, A transportAddress,
